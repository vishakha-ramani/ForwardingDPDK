...
#define RTE_HASH_RCU_DQ_ENTRY_TYPE_HASH_ENTRY 0
#define RTE_HASH_RCU_DQ_ENTRY_TYPE_VALUE 1
...
//					__ATOMIC_RELEASE);
                                orig_data = __atomic_exchange_n(&k->pdata,
                                                                data,
                                                                __ATOMIC_RELEASE);
                                printf("search_and_update orig_data=%p, data=%p, k->pdata=%p\n", orig_data, data, k->pdata);
                                if (h->hash_rcu_cfg) {
                                    rcu_dq_entry.type = RTE_HASH_RCU_DQ_ENTRY_TYPE_VALUE;
                                    rcu_dq_entry.entry.pointer = orig_data;
                                    if (h->dq == NULL) {
                                        /* Wait for quiescent state change if using
                                         * RTE_HASH_QSBR_MODE_SYNC
                                         */
                                        printf("Using SYNC mode \n");
...
__hash_rcu_qsbr_free_resource(void *p, void *e, unsigned int n)
{
	...
	RTE_SET_USED(n);
        if (rcu_dq_entry.type == RTE_HASH_RCU_DQ_ENTRY_TYPE_HASH_ENTRY) { //hash entry, original logic
           keys = h->key_store;
           
           k = (struct rte_hash_key *) ((char *)keys +
				rcu_dq_entry.entry.hash.key_idx * h->key_entry_size);
            key_data = k->pdata;
            if (h->hash_rcu_cfg->free_key_data_func)
                    h->hash_rcu_cfg->free_key_data_func(h->hash_rcu_cfg->key_data_ptr,
                                                        key_data);

            if (h->ext_table_support && rcu_dq_entry.entry.hash.ext_bkt_idx != EMPTY_SLOT)
                    /* Recycle empty ext bkt to free list. */
                    rte_ring_sp_enqueue_elem(h->free_ext_bkts,
                            &rcu_dq_entry.entry.hash.ext_bkt_idx, sizeof(uint32_t));

            /* Return key indexes to free slot ring */
            ret = free_slot(h, rcu_dq_entry.entry.hash.key_idx);
            if (ret < 0) {
                    RTE_LOG(ERR, HASH,
            ...
            if (h->hash_rcu_cfg->free_key_data_func) { // value only, added
                h->hash_rcu_cfg->free_key_data_func(h->hash_rcu_cfg->key_data_ptr, rcu_dq_entry.entry.pointer);
            }
        }
}
...
return_key:
	...
	if (h->hash_rcu_cfg) {
		/* Key index where key is stored, adding the first dummy index */
                rcu_dq_entry.type = RTE_HASH_RCU_DQ_ENTRY_TYPE_HASH_ENTRY;
		rcu_dq_entry.entry.hash.key_idx = ret + 1;
		rcu_dq_entry.entry.hash.ext_bkt_idx = index;
		if (h->dq == NULL) {
			/* Wait for quiescent state change if using
			 * RTE_HASH_QSBR_MODE_SYNC
			 */
